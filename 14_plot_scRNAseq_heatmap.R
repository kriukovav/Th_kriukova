...
plot_cluster_RNA + plot_RNA + plot_cells_RNA + plot_layout(heights = c(1, 7, 1))
ggsave(here("outs", "scRNAseq", "figures", "plot_RNA_combined.pdf"), height = 8, width = 11, dpi = 600)

# downsample data for heatmap sortseq -------------------------------------
...
intgr_for_sortseq <- subset(intgr_for_sortseq, cells = sample(1:length(rownames(intgr_for_sortseq[[]])), 10000))
...
plot_data %>%
  pivot_longer(cols = starts_with("sortseq") & ends_with("trb"), names_to = "Sort_seq_phenotype", values_to = "freq") -> plot_data_long
...
plot_data_long %>%
  ...
  theme(legend.position = "none",
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank()) -> plot_cluster_sortseq
...
ggplot(plot_data_long) +
  ...
  theme(legend.position = "bottom",
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank()) -> plot_cells_sortseq

plot_data_long %>%
  mutate(Sort_seq_phenotype = str_remove_all(.$Sort_seq_phenotype, "sortseq\\_|\\_trb")) %>%
  mutate(Sort_seq_phenotype = str_replace(.$Sort_seq_phenotype, "\\.", "\\-")) %>%
  mutate(Sort_seq_phenotype = factor(Sort_seq_phenotype, levels = c("Tfh", "TREG", "Th2a", "Th2", "Th22", "Th17", "Th1-17", "Th1"))) %>%
  ggplot() +
  geom_tile(aes(x = tile_order, y = Sort_seq_phenotype, fill = as.character(freq)), color = NA) +
  theme_bw() +
  ...
  theme(legend.position = "none",
        ...
        axis.ticks = element_blank(),
        axis.title = element_blank()) +
  geom_vline(xintercept = cluster_borders$cluster_border, color = "grey70", size = 0.5, linetype = "dashed") -> plot_sortseq

plot_cluster_sortseq + plot_sortseq + plot_cells_sortseq + plot_layout(heights = c(1, 7, 1))
ggsave(here("outs", "scRNAseq", "figures", "plot_Sortseq_combined.pdf"), height = 8, width = 11, dpi = 600)
...
plot_cluster_sortseq + plot_sortseq + plot_cluster_RNA + plot_RNA + plot_cluster_protein + plot_PROTEIN + plot_layout(ncol = 1, heights = c(1, 8, 1, 10, 1, 6)) 
