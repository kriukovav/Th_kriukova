...
neighbors %>%
  mutate(name = paste(cdr3aa, v, j, sep = "_")) %>%
  select(name, cdr3aa, v, j) %>%
  inner_join(df_clusters_membership) -> vertexes_cluster_ids 
...
vd_data <- vertexes_cluster_ids %>%
  ...
  mutate(meta.assay = case_when(meta.assay %in% c("COVID1_response", "COVID2_response", "VAX_response") ~ "timetracking",
                                TRUE ~ meta.assay)) %>%
  filter(!is.na(.$meta.assay)) %>%
  split(f = .$meta.assay) %>%
  map(function(x) select(x, cluster_id) %>% pull %>% unique)

vd_data <- vd_data[sort(names(vd_data))]
  
plot <- ggVennDiagram(vd_data, label = "count") + 
  scale_x_continuous(expand = expansion(mult = .2)) + 
  scale_y_continuous(expand = expansion(mult = .2)) + 
  scale_fill_gradient(low="white",high = "pink") +
  theme(legend.position = "none") +
  ggtitle("Clusters of similar TCRs verified by the method")

ggsave(plot = plot, filename = "venn_for_clusters.pdf", path = here("outs", "identified_clones", "figures"), height = 7, width = 7)
write_rds(plot, paste(here("outs", "identified_clones", "figures"), "venn_for_clusters.rds", sep = "/"))
